/**
 * @brief Implementation of the render system.
...
MAKE_VISIT_IMPL(Camera, {
    ADD_VAR_T(matrix);
    ADD_VAR_T(projection_matrix);
});
MAKE_VISIT_IMPL(VertexArray, {
    ADD_VAR2_LONG(updates_frequently, "updates frequently");
    ADD_VAR_T(data);
    ADD_VAR2_T(index_data, "faces");
    ADD_VAR_T(attributes);
    ADD_VAR2_LONG(draw_type, "draw_mode");
    t.recalc_stride();
});
MAKE_VISIT_IMPL(Light,{
               ADD_BASE(Node);
               ADD_VAR_DOUBLE(distance);
               ADD_VAR_DOUBLE(specular_power);
               ADD_VAR_DOUBLE(diffuse_power);
           })
...
MAKE_VISIT_IMPL(Material,{
               ADD_VAR_LONG(blend);
               ADD_VAR_LONG(render_flags);
               ADD_VAR_T(shader);
               ADD_VAR_T(color);

               ADD_VAR2_T(uniform.textures,"textures");
               ADD_VAR_T(name);
               ...
               if(!v.read_only()) {
                   while(v.has_unhandled_keys()) {
                       ...
                       if(s=="transparent"){
                           int x =0;
                           HANDLE_WITH_PICKER(v,x);
                           if(x&&t.blend==kBlendReplace){
                               t.blend=kBlendAlpha;
                           }
                       }else HANDLE_WITH_PICKER(v,t.uniform.get_fvec4(s));
                   }
               }
           })
MAKE_VISIT_IMPL(Renderable,{
               ...
               ADD_VAR_LONG(render_flags);
               ADD_VAR_T(tag);
               ADD_VAR_LONG(end_vertex);
               ADD_VAR_LONG(cull_face);
               ADD_VAR_T(vertex_array);
               ADD_VAR_T(material);
           })
};
