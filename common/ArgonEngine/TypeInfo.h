/**
 * @brief This file contains the classes necessary to define a class' type information for use with the Reflection System.
 * @file TypeInfo.h
...
#define ADD_VAR_T(A) v.handle_T(t.A,#A)
...
#define ADD_VAR2_T(A,B) v.handle_T(t.A,B)

#define ADD_VAR_LONG(A) v.handle_Long(t.A,#A)

#define ADD_VAR2_LONG(A,B) v.handle_Long(t.A,B)

#define ADD_VAR_DOUBLE(A) v.handle_Double(t.A,#A)

#define ADD_VAR2_DOUBLE(A,B) v.handle_Double(t.A,#A)
...
struct Visitor {
     ...
     **/
    template<typename T>
    void handle_T(T& t, const char* str){
        set_key(str);
        HandleIMPL<T,
                   TypeInfo<T>::valid,
                   false,
    ...
    void handle_Long(long t,const char*str){
        set_key(str);
        HandleIMPL<long,
                TypeInfo<long>::valid,
                std::numeric_limits<long>::is_specialized,
                std::numeric_limits<long>::is_integer,
                std::is_base_of<ReflectionBase,long>::value
                >::go(*this,t);
    }

    void handle_Long(long t){
        HandleIMPL<long,
                TypeInfo<long>::valid,
                std::numeric_limits<long>::is_specialized,
                std::numeric_limits<long>::is_integer,
    ...
    void handle_Double(double t,const char*str){
        set_key(str);
        HandleIMPL<double,
                TypeInfo<double>::valid,
                std::numeric_limits<double>::is_specialized,
                std::numeric_limits<double>::is_integer,
                std::is_base_of<ReflectionBase,double>::value
                >::go(*this,t);
    }

    void handle_Double(double t){
        HandleIMPL<double,
                TypeInfo<double>::valid,
                std::numeric_limits<double>::is_specialized,
                std::numeric_limits<double>::is_integer,
